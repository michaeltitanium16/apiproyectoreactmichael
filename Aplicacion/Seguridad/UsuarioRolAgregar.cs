           ...
           private readonly UserManager<Usuario> _userManager;
            ...
            public async Task<Unit> Handle(Ejecuta request, CancellationToken cancellationToken)
            {
               ...
               var user = await this._userManager.FindByNameAsync(request.Username);
                ...
                var resultado = await this._userManager.AddToRoleAsync(user, request.RolNombre);
                if (resultado.Succeeded)
                {
                   return  Unit.Value;
                }

                throw new ManejadorExcepcion(System.Net.HttpStatusCode.BadRequest, new { mensaje = "No se pudo agregar el rol al usuario" });
            }
        }
    }
}
